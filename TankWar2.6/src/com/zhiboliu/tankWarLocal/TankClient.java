package com.zhiboliu.tankWarLocal;
...
//2.3 add my tank life value

//2.4 make life value visualized

//2.5 add blood, after eating it, life value will return to 100

//2.6 when enemy tanks all die, they should join again, make F2 to restart the game
...
public class TankClient extends Frame{//extends Frame so u can draw a canvas
   ...
   // int x = 50, y = 50;//tank's beginning position
    ...
    //com.zhiboliu.tankWarLocal.Tank enemyTank = new com.zhiboliu.tankWarLocal.Tank(100, 100, false, this);
    ...
    // com.zhiboliu.tankWarLocal.Missile m = null;// missile should be initialized when control is clicked 1.3 deal with only one missile can be fired
    ...
    public void lauchFrame(){
        ...
        this.setTitle("com.zhiboliu.tankWarLocal.Tank War game is locally running. ");
...
//        //y += 5;  //test if thread can move the tank
        ...
        g.drawString("tank's life count: " + myTank.getLife(),10, 110);

        if(tanks.size() <= 0){// make new enemy tanks when all die
            for(int i = 0; i < 5; i++){
                tanks.add(new Tank(50 + 40 * (i + 1), 50, false, Tank.Direction.D, this));
            }
        }
